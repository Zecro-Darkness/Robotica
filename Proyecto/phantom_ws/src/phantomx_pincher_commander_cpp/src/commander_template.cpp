...
private:
    ...
    void planAndExecute(const std::shared_ptr<MoveGroupInterface> &interface)
    {
        ...
        if (success) {
            auto exec_code = interface->execute(plan);
            if (exec_code != moveit_msgs::msg::MoveItErrorCodes::SUCCESS) {
                 RCLCPP_ERROR(node_->get_logger(), "Execution failed with error code: %d", exec_code.val);
            } else {
                 RCLCPP_INFO(node_->get_logger(), "Execution successful!");
            }
        } else {
             RCLCPP_ERROR(node_->get_logger(), "Planning failed with error code: %d", error_code.val);
        }
    }
